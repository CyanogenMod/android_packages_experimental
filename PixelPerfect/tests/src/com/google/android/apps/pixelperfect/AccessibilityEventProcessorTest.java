...
public class AccessibilityEventProcessorTest extends AndroidTestCase {
    ...
    public void testCreateUIElement() {
        ...
        when(mNode.getChildCount()).thenReturn(2);
        ...
        checkElement(element, WidgetType.CHECKED_TEXT_VIEW, VIEW_ID_RESOURCE_NAME_1, TEXT_1, RECT);

        List<UIElement> childList = element.getChildList();
        assertEquals(2, childList.size());
        checkElement(childList.get(0), WidgetType.LINEAR_LAYOUT, null, TEXT_2, new Rect());
        checkElement(childList.get(1), WidgetType.EDIT_TEXT, null, null, RECT_B);
    }

    private void checkElement(UIElement element, WidgetType classType, String resourceName,
            String content, Rect rect) {
